...
export async function d20Roll({
  ...
  const _roll = (parts: any[], adv: number, form?: any) => {
    ...
    if (adv === 1) {
      nd = 2;
      messageData.flavor += ` (${game.i18n.localize('CarRoy.Advantage')})`;
      if ('flags.carroy.roll' in messageData) messageData['flags.carroy.roll'].advantage = true;
      mods += 'kh';
    }
    ...
    // Handle disadvantage
      ...
      nd = 2;
      messageData.flavor += ` (${game.i18n.localize('CarRoy.Disadvantage')})`;
      if ('flags.carroy.roll' in messageData) messageData['flags.carroy.roll'].disadvantage = true;
      mods += 'kl';
    }
    ...
    if (ability && ability.value) {
      ...
      if (abl) {
        data.mod = abl.mod;
        messageData.flavor += ` (${CONFIG.CarrotRoyale.abilities[data.ability]})`;
      }
    }
 ...
 * @private
 */
async function _d20RollDialog({ template, title, parts, data, rollMode, dialogOptions, roll }: RollDialog = {}): Promise<Roll | null> {
  // Render modal dialog
  template = template || 'systems/carroy/templates/chat/roll-dialog.html';
  let dialogData = {
    formula: parts?.join(' + '),
    data: data,
    rollMode: rollMode,
    ...
    new Dialog(
      {
        ...
        buttons: {
          advantage: {
            label: game.i18n.localize('CarRoy.Advantage'),
            callback: (html) => resolve(roll?.(parts, 1, (html as any)[0].querySelector('form'))),
          },
          normal: {
            label: game.i18n.localize('CarRoy.Normal'),
            callback: (html) => resolve(roll?.(parts, 0, (html as any)[0].querySelector('form'))),
          },
          disadvantage: {
            label: game.i18n.localize('CarRoy.Disadvantage'),
            callback: (html) => resolve(roll?.(parts, -1, (html as any)[0].querySelector('form'))),
          },
        },
        default: 'normal',
...
export async function damageRoll({
  ...
  const _roll = function (parts: string[], crit: boolean = false, form?: any): Roll | null {
    // Optionally include a situational bonus
    if (form) {
      data['bonus'] = form.bonus.value;
      messageOptions.rollMode = form.rollMode.value;
    ...
    if (crit === true) {
      ...
      if (roll.terms[0] instanceof Die) {
        ...
        (roll.terms[0] as DiceTerm).alter(1, criticalBonusDice);
        roll._formula = roll.formula;
      }
      roll.dice.forEach((d) => (d.options.critical = true));
      messageData.flavor += ` (${game.i18n.localize('CarRoy.Critical')})`;
      if ('flags.carroy.roll' in messageData) messageData['flags.carroy.roll'].critical = true;
    }
  ...
  // Create the Roll instance
  const roll: Roll | null = fastForward
 ...
 * @private
 */
async function _damageRollDialog({ template, title, parts, data, allowCritical, rollMode, dialogOptions, roll }: RollDialog = {}): Promise<Roll | null> {
  // Render modal dialog
  template = template || 'systems/carroy/templates/chat/roll-dialog.html';
  let dialogData = {
    formula: parts?.join(' + '),
    ...
    new Dialog(
      {
        ...
        buttons: {
          critical: {
            ...
            label: game.i18n.localize('CarRoy.CriticalHit'),
            callback: (html) => resolve(roll?.(parts, true, (html as any)[0].querySelector('form'))),
          },
          normal: {
            label: game.i18n.localize(allowCritical ? 'CarRoy.Normal' : 'CarRoy.Roll'),
            callback: (html) => resolve(roll?.(parts, false, (html as any)[0].querySelector('form'))),
          },
        },
        default: 'normal',
        close: () => resolve(null),
